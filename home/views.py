...
def stats(request):
    if request.session.has_key('is_logged'):
        ...
        user_profile, created = UserProfile.objects.get_or_create(user=user1)

        # Ensure Savings is not None
        savings = user_profile.Savings or 0
        ...
        x = savings + total_income - total_expense
        y = x
...
def weekly(request):
    if request.session.has_key('is_logged'):
        ...
        # Pass data through context (not attached to QuerySet)
        context = {
    ...
    return redirect('login')  # Or another fallback page
